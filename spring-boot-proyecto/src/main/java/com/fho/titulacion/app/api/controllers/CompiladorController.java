		...
		if (resultado == "creado") {
			...
			try {
				...
				if (hex.exists() && hex.canRead()) {
					...
					response = new HexResponse(res, e, false, "");
				...
				if (err.exists() && err.canRead()) {
					...
					response = new HexResponse(null, null, true, errorString);
				...
				e.printStackTrace();
				response = new HexResponse(null, null, true, e.getMessage());
